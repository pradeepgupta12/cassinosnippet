...
const Home = () => {
  ...
  const [isPopupClosed, setIsPopupClosed] = useState(sessionStorage.getItem('popupClosed') === 'true');
  ...
  const [visibleHowToPlayCards, setVisibleHowToPlayCards] = useState(5);

  // Function to truncate description to ~2-3 lines (approx. 120 characters)
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 100 && !isPopupOpen && !isPopupClosed) {
        setIsPopupOpen(true);
      }
    };
    ...
    <div className="text-[#D3D3D3]">
          ...
          {[...gamesData.slots, ...gamesData.table, ...gamesData.live].map((game) => (
            ...
            >
              <img 
                src={game.image} 
                alt={`${game.name} image`} 
                className="h-40 w-full object-cover-contain object-center mb-2 rounded-t-lg bg-[#2E2E5A]"
              />
            ...
            <div className="grid grid-cols-1 md:grid-cols-1 gap-4 flex-grow">
              {guidesData.guides.slice(0, visibleGuideCards).map((guide) => (
                <Link
                  ...
                  className="p-6 rounded-lg shadow-xl bg-white border border-gray-200 hover:shadow-2xl hover:border-[#FFD700] transition-shadow duration-300"
                >
            ...
            <ul className="space-y-4 flex-grow">
              {guidesData.guides.slice(0, visibleHowToPlayCards).map((guide) => (
                <li key={guide.guideId} className="border-b border-gray-300 pb-2">
                  <Link to={`/guides/${guide.guideId}`} className="block group">
                    <span className="text-gray-600 text-sm group-hover:text-blue-600 transition-colors duration-200">{guide.title}</span>
                    <p className="text-gray-700 text-xs mt-1 group-hover:text-blue-600 transition-colors duration-200">{truncateDescription(guide.description)}</p>
                  </Link>
                </li>
              ))}
            </ul>
        ...
        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-6">
          {[...(casinoNewsAndStories.news?.filter(item => item.isFeatured) || []), ...(casinoNewsAndStories.stories?.filter(item => item.isFeatured) || []), ...(casinoNewsAndStories.news?.filter(item => !item.isFeatured) || []), ...(casinoNewsAndStories.stories?.filter(item => !item.isFeatured) || [])].slice(0, visibleNewsCards).map((item) => (
            <Link
              ...
              className="rounded-lg bg-white shadow-md border border-gray-200 hover:shadow-lg duration-300 flex flex-col md:flex-row min-h-[150px] p-0 cursor-pointer hover:border-[#FFD700] hover:-translate-y-2 transition-all"
            >
              <img src={item.image_url} alt={`${item.title} image`} className="w-full md:w-1/3 h-32 md:h-full object-cover-contain rounded-lg" />
      ...
      <section className="py-12 px-4 md:px-8 max-w-7xl mx-auto bg-white text-[#333333] overflow-visible">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {casinoBlogs.blogs.slice(0, 3).map((blog) => (
            <Link
              key={blog.id}
      ...
      {isPopupOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg text-center w-full max-w-md relative">
            <button
              onClick={() => { setIsPopupOpen(false); setIsPopupClosed(true); sessionStorage.setItem('popupClosed', 'true'); window.scrollTo(0, 0); }}
