...
export default function ContactPage() {
  ...
  return (
    <div className="bg-[#F5F5F5] text-[#333333] py-10 px-4">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-3xl md:text-4xl font-bold text-center text-[#FFD700] mb-10">
          Contact Us
        </h2>
        <div className="grid md:grid-cols-3 gap-6 text-center mb-12">
          <div>
            <IoMail className="text-[#FFD700] text-2xl mx-auto mb-2" />
            <p>support@casinosite.com</p>
          </div>
          <div>
            <IoCall className="text-[#FFD700] text-2xl mx-auto mb-2" />
            <p>+1 234 567 890</p>
          </div>
          <div>
            <IoLocation className="text-[#FFD700] text-2xl mx-auto mb-2" />
            <p>Las Vegas, Nevada, USA</p>
          </div>
        </div>
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          >
            <div className="grid md:grid-cols-2 gap-4">
              <input
                ...
                value={formData.name}
                onChange={handleChange}
                className="w-full p-3 rounded border border-[#1A1A3D] bg-white text-[#333333] focus:outline-none"
              />
              <input
                type="email"
                name="email"
            ...
            <label className="flex items-start gap-2 text-sm">
              ...
              />
              I agree to the{" "}
              <a href="/privacy" className="text-[#E63946] underline">
                Privacy Policy
              </a>
            </label>
            <button
              type="submit"
              className="bg-[#FFD700] hover:bg-[#FFC107] text font-bold py-2 px-6 rounded transition"
            >
              Submit
            </button>
          </form>
          <div className="bg-[#F5F5F5] p-6 md:p-8 rounded-lg shadow">
            <h3 className="text-xl font-bold mb-6 text-center text-[#333333]">
              Frequently Asked Questions
            </h3>
            <div className="space-y-4">
              {faqs.map((faq, index) => (
                <div key={index} className="border border-[#1A1A3D] rounded">
                  <button
                    onClick={() => toggleFAQ(index)}
                    className="w-full flex justify-between items-center p-4 text-left font-medium text-[#333333]"
                  >
                    {faq.question}
                    {openIndex === index ? (
                      <IoRemove className="text-[#FFD700]" />
                    ) : (
                      <IoAdd className="text-[#FFD700]" />
                    )}
                  </button>
                  {openIndex === index && (
                    <div className="p-4 pt-0 text-gray-700 text-sm">
