...
import { baccaratCasinos } from '../Data/baccaratCasinos';
import { pokerCasinos } from '../Data/pokerCasinos';

// Merge casino data, allowing same casino.id to appear for different game types
const allCasinos = [
  ...blackjackCasinos.map(casino => ({ ...casino, logo: casino.logo || casino.image, country: casino.country || casino.region, gameType: 'Blackjack' })),
  ...baccaratCasinos.map(casino => ({ ...casino, logo: casino.logo || casino.image, country: casino.country || casino.region, gameType: 'Baccarat' })),
  ...pokerCasinos.map(casino => ({ ...casino, logo: casino.logo || casino.image, country: casino.country || casino.region, gameType: 'Poker' })),
  ...pokerCasinos.map(casino => ({ ...casino, logo: casino.logo || casino.image, country: casino.country || casino.region, gameType: 'Dice' })),
];
...
const AllGames = () => {
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero Section (updated for All Games) */}
      <div className="bg-gradient-to-r from-[#1A1A3D] to-[#2E2E5A] py-12 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-lg sm:text-xl text-white/90 mb-6 max-w-2xl mx-auto">
            Explore the best online casinos for Blackjack, Baccarat, Poker, and Dice games in India. Play with trusted operators and enjoy exclusive bonuses.
          </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {allCasinos.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">Check back later for more casino options!</p>
            </div>
          ) : (
            <div className="space-y-4 sm:space-y-6">
              {allCasinos.map((casino) => (
                <CasinoCard key={`${casino.id}-${casino.gameType}`} casino={casino} />
              ))}
            </div>
          )}
        </div>
